...
@onready var player_dectection_area: Area2D = $PlayerDectionArea
var active_enemy_counter := 0
var enemy_data : Array[EnemyData] = []
var is_activated := false
func _ready() -> void:
	player_dectection_area.body_entered.connect(on_player_enter.bind())
	EntityManager.death_enemy.connect(on_enemy_death.bind())
func _process(_delta:float) -> void:
	if is_activated and enemy_data.size() > 0:
