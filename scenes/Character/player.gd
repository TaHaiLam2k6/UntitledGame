...
func _ready() -> void:
	super._ready()
	anim_attacks = ["punch", "punch_alt", "heavy", "heavy_alt"]
...
func handle_input() -> void:
	...
	if can_jump() and Input.is_action_just_pressed("jump"):
		state = State.TAKEOFF
		attack_combo_index = 0
	if can_jumphit() and Input.is_action_just_pressed("attack"):
		state = State.JUMPHIT
		SoundPlayer.play(SoundManager.Sound.SWOOSH)
...
func reserve_slot(enemy: Enemy) -> EnemySlot:
	...
	available_slots.sort_custom(
		func(a: EnemySlot, _b: EnemySlot):
			var dist_a := (enemy.global_position - a.global_position).length()
			var dist_b := (enemy.global_position - a.global_position).length()
